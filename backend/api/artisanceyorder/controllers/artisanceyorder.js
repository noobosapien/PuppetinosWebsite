...
const stripe = require("stripe")(process.env.ARTISANCEY_SK_LIVE);
...
module.exports = {
  async place(ctx) {
    ...
    try {
      ...
      const artisanceyOrders = strapi.services["artisanceyorder"];
      var order = await artisanceyOrders.create(orderObject);

      order = sanitizeEntity(order, {
        model: strapi.models["artisanceyorder"],
      });

      const confirmation = await artisanceyOrders.confirmationEmail(
        orderObject
      );
  ...
  async process(ctx) {
    ...
    try {
      ...
      const artisanceyProducts = strapi.services["artisanceyproducts"];

      await Promise.all(
        items.map(async (clientItem) => {
          const serverItem = await artisanceyProducts.findOne({
            id: clientItem.id,
          });
